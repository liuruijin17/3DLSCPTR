    ...
    for b_ind in range(batch_size):
        ...
        if lane_debug:
            ...
            for i, lane in enumerate(draw_2dgflatlabels):
                ...
                gt_2dlane    = lane[:3+seq_len*2]
                gt_3dlane    = lane[3+seq_len*2:5+seq_len*5]
                gt_gflatlane = lane[5+seq_len*5:]
                lane = gt_2dlane[3:]
                ...
                gflatXs = gt_gflatlane[:seq_len][draw_2dgflatflags[i] > 0] * db.gflatXnorm
                gflatYs = gt_gflatlane[seq_len:seq_len*2][draw_2dgflatflags[i] > 0] * db.gflatYnorm
                gflatZs = gt_gflatlane[seq_len*2:][draw_2dgflatflags[i] > 0] * db.gflatZnorm
                ...
                orgXS = gt_3dlane[2:2+seq_len][draw_2dgflatflags[i] > 0] * db.gflatXnorm
                orgYS = gt_3dlane[2+seq_len:2+seq_len*2][draw_2dgflatflags[i] > 0] * db.gflatYnorm
                orgZS = gt_3dlane[2+seq_len*2:2+seq_len*3][draw_2dgflatflags[i] > 0] * db.gflatZnorm

                ax3.plot(orgXS, orgYS, orgZS, color=[1, 0, 1])
            ax3.set_xlabel('X')
            ax3.set_ylabel('Y')
            ax3.set_zlabel('Z')
